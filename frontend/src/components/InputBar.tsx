...
export default function InputBar() {
  ...
  const hasSubmitAccessToken = Boolean(settings.accessToken.trim())
  const canSubmit = Boolean(prompt.trim() && hasSubmitAccessToken)
  const moderationDisabled = settings.apiMode === 'responses'
  const compressionDisabled = params.output_format === 'png'
  const outputImageLimit = getOutputImageLimitForSettings(effectiveSettings)
  const nLimitHintText = `当前最大请求数量为 ${outputImageLimit}`
          ...
          <div className="mt-3">
            ...
            <div className="hidden sm:flex items-end justify-between gap-3">
              ...
              <div className="flex gap-2 flex-shrink-0 mb-0.5">
                ...
                >
                  <ButtonTooltip visible={!hasSubmitAccessToken && submitHover} text="请先在设置中填写后端访问口令" />
                  <button
                    onClick={() => hasSubmitAccessToken ? submitTask() : setShowSettings(true)}
                    disabled={hasSubmitAccessToken ? !canSubmit : false}
                    className={`p-2.5 rounded-xl transition-all shadow-sm hover:shadow ${
                      !hasSubmitAccessToken
                        ? 'bg-gray-300 dark:bg-white/[0.06] text-white cursor-pointer'
                        : 'bg-blue-500 text-white hover:bg-blue-600 disabled:bg-gray-300 dark:disabled:bg-white/[0.04] disabled:opacity-50 disabled:cursor-not-allowed'
                    }`}
                    title={hasSubmitAccessToken ? (maskDraft ? '遮罩编辑 (Ctrl+Enter)' : '生成 (Ctrl+Enter)') : '请先填写访问口令'}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
